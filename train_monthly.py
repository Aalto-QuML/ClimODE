...
const_info_path = ['/scratch/project_2006852/PDE_climate/era5_data/constants/constants_5.625deg.nc']
...
model = Climate_encoder_free_uncertain_monthly_v3(len(paths_to_data),2,out_types=len(paths_to_data),method=args.solver,use_att=True,use_err=True,use_pos=False).to(device)
...
for epoch in range(args.niters):
    ...
    if val_loss < best_loss:
        best_loss = val_loss
        best_epoch = epoch
        torch.save(model,str(cwd) + "/Models/" + "ClimODE_monthly_avg_"+args.solver+"_"+str(args.spectral)+"_model_" + str(epoch) + ".pt")

    if total_train_loss < train_best_loss:
        train_best_loss = total_train_loss
        torch.save(model,str(cwd) + "/Models/" + "ClimODE_monthly_avg_overfit_"+args.solver+"_"+str(args.spectral)+"_model_" + str(epoch) + ".pt")
